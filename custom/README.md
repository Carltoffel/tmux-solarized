...
2. Copy the following template to this new file. Make sure to replace every instance of `<module_name>` by the name you chose as filename.
    ...
    show_<module_name>() { # This function name must match the module name!
      ...
      icon="$(  get_tmux_option "@solarized_<module_name>_icon"  ""           )"
      color="$( get_tmux_option "@solarized_<module_name>_color" "$thm_orange" )"
      text="$(  get_tmux_option "@solarized_<module_name>_text"  "hello world" )"
...
3. Add the custom module to the list of modules in `.tmux.conf`
    ...
    ```bash
    set -g @solarized_status_modules_right "... <module_name> ..."
    ```
...
Change the color to one of the [official colors](../solarized-dark.tmuxtheme), for instance `"$thm_cyan"`, or to a hexadecimal color like `"#00ff00"`.
...
```tmux
set -g @solarized_custom_plugin_dir "<path>"
```
